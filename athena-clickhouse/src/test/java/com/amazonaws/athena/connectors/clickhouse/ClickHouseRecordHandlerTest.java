...
public class ClickHouseRecordHandlerTest {
            ...
            throws Exception
    {
        ...
        final DatabaseConnectionConfig databaseConnectionConfig = new DatabaseConnectionConfig("testCatalog", MYSQL_NAME,
                "clickhouse://jdbc:clickhouse://hostname/user=A&password=B");

        this.recordHandler = new ClickHouseRecordHandler(databaseConnectionConfig, amazonS3, secretsManager, athena, jdbcConnectionFactory, jdbcSplitQueryBuilder, com.google.common.collect.ImmutableMap.of());
    }
    ...
    @Test
    public void passthroughSql()
            throws SQLException
    {

        String expectedSql = "SELECT * FROM TABLE(system.query(query => 'SELECT AVG(testCol1), testCol2 FROM testSchema.testTable WHERE 1=1 GROUP BY testCol2 ORDER BY testCol2 ASC LIMIT 100'))";
        ...
        HashMap<String, String> queryPassthroughArguments = new HashMap<>(3);

        queryPassthroughArguments.put("QUERY", expectedSql);
        queryPassthroughArguments.put("ARGUMENTS", "");
        ...
        // SELECT * FROM TABLE(system.query(query => 'SELECT AVG(testCol1), testCol2 FROM testSchema.testTable WHERE 1=1 GROUP BY testCol2 ORDER BY testCol2 ASC LIMIT 100";'))
        // "SELECT AVG(testCol1), testCol2 FROM testSchema.testTable WHERE 1=1 GROUP BY testCol2 ORDER BY testCol2 ASC LIMIT 100"
